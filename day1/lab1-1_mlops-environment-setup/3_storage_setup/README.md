# Lab 1-1 Part 3: 스토리지 확인

## 📋 개요

이 섹션에서는 MLOps 플랫폼의 스토리지 구성을 확인합니다.

**소요시간:** 15분

---

## 🎯 확인 항목

- **MinIO**: S3 호환 객체 스토리지 (MLflow Artifacts)
- **PostgreSQL**: 관계형 데이터베이스 (MLflow Metadata)

---

## 🏗️ 스토리지 아키텍처

```
┌─────────────────────────────────────────────────────────┐
│                   MLOps Platform                        │
├─────────────────────────────────────────────────────────┤
│                                                         │
│  ┌─────────────────┐         ┌──────────────────┐     │
│  │   Kubeflow      │         │     MLflow       │     │
│  │   Pipeline      │────────▶│  Tracking Server │     │
│  │                 │         │    (Port 5000)   │     │
│  └─────────────────┘         └─────────┬────────┘     │
│          │                              │              │
│          │                    ┌─────────▼────────┐    │
│          │                    │   PostgreSQL     │    │
│          │                    │   (Metadata)     │    │
│          │                    │    Port 5432     │    │
│          │                    └──────────────────┘    │
│          │                                             │
│  ┌───────▼────────┐                                   │
│  │     MinIO      │◀─────────────────────────────────│
│  │  (Artifacts)   │         (Artifact Store)          │
│  │   Port 9000    │                                   │
│  └────────────────┘                                   │
│                                                        │
└────────────────────────────────────────────────────────┘
```

---

## 🚀 단계별 실행

### Step 1: 자동 검증 스크립트 실행

```bash
# 검증 스크립트 실행
./3_storage_setup/verify_storage.sh
```

**예상 출력:**
```
============================================================
Lab 1-1 Part 3: 스토리지 확인
============================================================

============================================================
Step 1: MinIO 파드 상태 확인
============================================================
✅ MinIO 파드: minio-xxxxxxxxxx-xxxxx
NAME                     READY   STATUS    RESTARTS   AGE
minio-xxxxxxxxxx-xxxxx   1/1     Running   0          30d

============================================================
Step 2: MinIO 서비스 확인
============================================================
✅ MinIO 서비스 존재
minio-service   ClusterIP   10.100.180.100   <none>   9000/TCP,9001/TCP   30d

============================================================
Step 3: PostgreSQL 파드 상태 확인
============================================================
✅ PostgreSQL 파드: postgres-xxxxxxxxxx-xxxxx
NAME                        READY   STATUS    RESTARTS   AGE
postgres-xxxxxxxxxx-xxxxx   1/1     Running   0          30d

============================================================
Step 4: PostgreSQL 서비스 확인
============================================================
✅ postgres-service 존재
postgres-service   ClusterIP   10.100.150.101   <none>   5432/TCP   30d

============================================================
Step 5: 스토리지 아키텍처 요약
============================================================

📊 스토리지 구성:

  ┌─────────────────────────────────────────────┐
  │         MLOps Storage Architecture          │
  ├─────────────────────────────────────────────┤
  │                                             │
  │  MinIO (S3-Compatible Object Storage)       │
  │  ├─ Namespace: kubeflow                     │
  │  ├─ Port: 9000 (API), 9001 (Console)        │
  │  └─ 용도: MLflow Artifacts, Pipeline Data   │
  │                                             │
  │  PostgreSQL (Relational Database)           │
  │  ├─ Namespace: mlflow-system                │
  │  ├─ Port: 5432                              │
  │  └─ 용도: MLflow Metadata                   │
  │                                             │
  └─────────────────────────────────────────────┘

============================================================
✅ 스토리지 확인 완료!
============================================================
```

### Step 2: MinIO Console 접속 (선택사항)

```bash
# MinIO Console 포트 포워딩
kubectl port-forward svc/minio-service -n kubeflow 9000:9000 9001:9001
```

**브라우저 접속:**
```
http://localhost:9001
```

**로그인 정보:**
- Username: `minio`
- Password: `minio123`

**MinIO Console에서 확인:**
1. **Buckets**: MLflow artifact 버킷 확인
2. **Browse**: 저장된 파일 확인
3. **Monitoring**: 스토리지 사용량 확인

### Step 3: PostgreSQL 연결 테스트 (선택사항)

```bash
# PostgreSQL 파드에 접속
kubectl exec -it deployment/postgres -n mlflow-system -- psql -U mlflow

# 데이터베이스 목록 확인
\l

# 테이블 목록 확인
\dt

# Experiments 테이블 확인
SELECT * FROM experiments;

# 종료
\q
```

---

## 📊 스토리지 역할

### MinIO (S3-Compatible Object Storage)

**위치:**
- Namespace: `kubeflow`
- 포트: 9000 (S3 API), 9001 (Console)

**저장 내용:**
- MLflow Artifacts (모델 파일, 그래프, 데이터)
- Kubeflow Pipeline Artifacts
- 학습 결과물

**특징:**
- S3 API 완전 호환
- 대용량 파일 저장 최적화
- 버전 관리 지원

### PostgreSQL (Relational Database)

**위치:**
- Namespace: `mlflow-system`
- 포트: 5432

**저장 내용:**
- MLflow 메타데이터
  - Experiments 정보
  - Runs 정보
  - Parameters, Metrics, Tags
  - Model Registry 정보

**특징:**
- ACID 트랜잭션 보장
- 관계형 데이터 모델
- 고속 쿼리 성능

---

## 🔗 데이터 흐름

```
1. 학습 실행
   └─▶ MLflow Tracking
       ├─▶ PostgreSQL: Parameters, Metrics 저장
       └─▶ MinIO: Model 파일, Artifacts 저장

2. 모델 조회
   ├─▶ PostgreSQL: 메타데이터 조회
   └─▶ MinIO: 모델 파일 다운로드

3. 파이프라인 실행
   ├─▶ MinIO: 입력 데이터 읽기
   └─▶ MinIO: 결과 저장
```

---

## 💡 문제 해결

### 문제: MinIO 파드를 찾을 수 없음

**확인:**
```bash
# 모든 네임스페이스에서 검색
kubectl get pods --all-namespaces | grep minio

# 특정 네임스페이스 확인
kubectl get pods -n kubeflow
```

### 문제: PostgreSQL 연결 실패

**확인:**
```bash
# 파드 로그 확인
kubectl logs -l app=postgres -n mlflow-system --tail=50

# 파드 상세 정보
kubectl describe pod -l app=postgres -n mlflow-system
```

### 문제: MinIO Console 접속 불가

**확인:**
```bash
# 포트 포워딩 상태
ps aux | grep "port-forward"

# 서비스 상태 확인
kubectl get svc -n kubeflow | grep minio

# 포트 포워딩 재시작
pkill -f "port-forward.*minio"
kubectl port-forward svc/minio-service -n kubeflow 9000:9000 9001:9001
```

---

## ✅ 완료 체크리스트

- [ ] MinIO 파드 Running 상태 확인
- [ ] MinIO 서비스 확인
- [ ] PostgreSQL 파드 Running 상태 확인
- [ ] PostgreSQL 서비스 확인
- [ ] 스토리지 아키텍처 이해
- [ ] (선택) MinIO Console 접속 확인
- [ ] (선택) PostgreSQL 연결 테스트

---

## 🎯 학습 성과

이 섹션을 완료하면:

1. ✅ **MinIO 아키텍처** - S3 호환 객체 스토리지
2. ✅ **PostgreSQL 구성** - 메타데이터 관리
3. ✅ **스토리지 역할 분담** - Artifacts vs Metadata
4. ✅ **데이터 흐름** - 학습 → 저장 → 조회
5. ✅ **MLOps 인프라** - 전체 스토리지 계층 이해

---

## 📚 다음 단계

**Lab 1-2: Hello World Pipeline** - Kubeflow Pipelines로 첫 번째 ML 워크플로우 작성

---

© 2024 현대오토에버 MLOps Training
